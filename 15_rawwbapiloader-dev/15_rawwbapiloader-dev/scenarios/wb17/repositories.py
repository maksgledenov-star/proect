...
class WB17Repository(BaseDBRepository):
        ...
        try:
            with self.connection as conn:
                ...
                with conn.cursor() as cursor:
                    cursor.execute("SELECT CAST(SYSUTCDATETIME() AS datetime2(3))")  # Fixed missing parenthesis
                ...
                with conn.cursor() as cursor:
                    cursor.executemany("""
                        INSERT INTO [RawWBData].[wb17_ProductsReport]
                            (Lpid, IsTestData, CollectionStartDttm, CollectionEndDttm,
                             NmID, ImtID, NmUUID, SubjectID, SubjectName,
                             VendorCode, Brand, Title, Description, NeedKiz, Video, Photos, Wholesale,
                             Dimensions, Characteristics, Sizes, Tags, CreatedAt, UpdatedAt)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
                    """, params)
                ...
                logger.info(f"Successfully inserted {len(params)} records",
                            extra={'log_data': {'event_code': 'INSERT_DATA_SUCCESS',
                                                'destination_table': 'wb17_ProductsReport'}}
                            )
